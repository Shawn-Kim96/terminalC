...
from terminalc.runtime_core.models.runtime_models import Intent, IntentSlots, QueryPlan, QuerySpec
...
class QueryOrchestrator:
    ...
    def __init__(
        ...
        self._candles_columns = list(DUCKDB_TABLE_COLUMNS.get("candles", ()))
        self._indicator_summary_columns = tuple(DUCKDB_TABLE_COLUMNS.get("indicator_signal_summary", ()))
        self._indicator_signal_columns = tuple(DUCKDB_TABLE_COLUMNS.get("indicator_signals", ()))
        self._divergence_columns = tuple(DUCKDB_TABLE_COLUMNS.get("divergence", ()))
        self._news_columns = tuple(DUCKDB_TABLE_COLUMNS.get("news_articles", ()))
        self._indicator_rule_columns = tuple(DUCKDB_TABLE_COLUMNS.get("indicator_rules", ()))
        self._strategy_columns = tuple(DUCKDB_TABLE_COLUMNS.get("strategies", ()))
        self._limits = {
            "candles": None,
    ...
    def _build_candles_spec(self, base_context: Mapping[str, Any], metrics: Sequence[str]) -> QuerySpec:
        ...
        start, end = self._normalize_time_bounds(start, end)
        if start:
    ...
    @staticmethod
    def _normalize_time_bounds(start: Any, end: Any) -> tuple[str | None, str | None]:
        def normalize(value: Any, is_end: bool) -> str | None:
            if value is None:
                return None
            ...
            if isinstance(value, date):
                dt = datetime.combine(value, time.max if is_end else time.min, tzinfo=timezone.utc)
            ...
            if isinstance(value, str):
                ...
                try:
                    ...
                    return text
                dt = datetime.combine(parsed, time.max if is_end else time.min, tzinfo=timezone.utc)
                return dt.isoformat().replace("+00:00", "Z")
            return str(value)

        return normalize(start, False), normalize(end, True)
